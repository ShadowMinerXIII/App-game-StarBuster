...
local function createCoins()
    ...
    --coins:setFillColor(250, 0, 0)
    cCounter = cCounter + 1
end

coinTimer = timer.performWithDelay(3000, createCoins, -1)

--local function createBombs()
  --  local fallBombs = math.random(display.contentWidth * -0.2, display.contentWidth * 0.7)
   -- bombs[bCounter] = display.newCircle(fallBombs, display.contentCenterY - 500, 10)
   -- physics.addBody(bombs[bCounter], "dynamic")
   -- bombs[bCounter]:setLinearVelocity( 0, 300)
   -- bombs[bCounter].myName = "bombs"
   -- bombs[bCounter].value = bCounter
   -- bCounter = bCounter + 1
--end

--bombTimer = timer.performWithDelay(3000, createBombs, -1)
...
local function moveShip( event )
	...
	if( "began" == phase ) then
        ...
        if event.x >150 and event.x<display.contentWidth - 110 then
		ship.touchOffsetX = event.x - ship.x
        end
        if event.y >60 and event.y<display.contentHeight + 150 then
    ...
    elseif ( "moved" == phase ) then
        ...
        if event.y >60 and event.y<display.contentHeight + 60 then
...
local scoreTimer = timer.performWithDelay(3000, updateScore, 0)
...
--local function bombHit(obj) 
  --    obj.myName = "bombHitShip"
    --  transition.to(obj, {alpha = 0, onComplete=function(self) display.remove(self); end})
     -- lives = lives - 1
     -- livesText.text = "Lives: " ..lives
     -- score = score -20
...
local function onCollision( event )
   ...
   -- if(obj1.myName == "ship" and obj2.myName == "bombs") or
   -- ( obj1.myName == "bombs" and obj2.myName == "ship" )then
   --   timer.performWithDelay(1, function() bombHit(obj1); end, 1) end
    ...
    if( died == false ) then
   	...
   	if (lives == 0 ) then
        ...
        timer.performWithDelay( 2000, endGame)
   		...
   		ship.alpha = 0
   		timer.performWithDelay( 2000, restartShip) 
              end 
		   end
		end
    end
